...
// تعريف الدالة RtlLookupPrivilegeValue (موجودة في ntdll.dll)
NTSTATUS NTAPI RtlLookupPrivilegeValue(
    _In_opt_ PCWSTR lpSystemName,
    _In_ PCWSTR lpName,
    _Out_ PLUID lpLuid
...
/**
 * Attempts to steal/duplicate a process handle from a SYSTEM-level process.
 * This technique bypasses usermode handle callbacks by finding an existing
 * open handle to the target process in a privileged process and duplicating it.
 *
 * \param ProcessHandle Receives the duplicated handle on success.
 * \param DesiredAccess The desired access mask.
 * \param ProcessId The target process ID.
 * \return NTSTATUS Successful or errant status.
 */
static NTSTATUS PhpStealProcessHandleFromSystem(
    ...
    )
{
    ...
    // Enumerate all system handles to find one pointing to our target process
    ...
    for (i = 0; i < handleInfo->NumberOfHandles; i++)
    {
        ...
        // Only look at Process object handles from high-privilege processes
        // (object type 7 is Process on most Windows versions)
        if (entry->ObjectTypeIndex != 7)
            continue;

        // Skip our own process and the target process themselves
        if ((HANDLE)(ULONG_PTR)entry->UniqueProcessId == NtCurrentProcessId())
            continue;
        if ((HANDLE)(ULONG_PTR)entry->UniqueProcessId == ProcessId)
            continue;

        // Check if this handle points to the same object as our target.
        // We use PROCESS_QUERY_LIMITED_INFORMATION to peek at the target object.
        {
            ...
            cid.UniqueProcess = (HANDLE)(ULONG_PTR)entry->UniqueProcessId;
            cid.UniqueThread = NULL;
            InitializeObjectAttributes(&oa, NULL, 0, NULL, NULL);

            // Open the source process (the one that holds a handle to our target)
            if (!NT_SUCCESS(NtOpenProcess(&sourceProcess, PROCESS_DUP_HANDLE, &oa, &cid)))
                continue;

            // Duplicate the handle from source into our process
            ...
            {
                // Verify this is actually the process we want
 ...
 * Opens a process.
 *
 * Uses a multi-stage bypass chain to open a handle even on protected processes:
 *   1. KPH kernel driver (if at medium level and access fits read profile)
 *   2. Normal NtOpenProcess
 *   3. SeDebugPrivilege escalation + retry
 *   4. Handle duplication from a SYSTEM/privileged process (handle stealing)
 *   5. KPH kernel driver at max level (last resort)
 *
    ...
    )
{
    ...
    // --- Stage 1: KPH driver at medium level (fast path for read access) ---
    ...
    // --- Stage 3: Enable SeDebugPrivilege and retry ---
    if (status == STATUS_ACCESS_DENIED)
    {
        HANDLE tokenHandle;
        if (NT_SUCCESS(NtOpenProcessToken(NtCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &tokenHandle)))
        {
            TOKEN_PRIVILEGES tp;
            LUID debugLuid;

            if (NT_SUCCESS(RtlLookupPrivilegeValue(NULL, SE_DEBUG_NAME, &debugLuid)))
            {
                tp.PrivilegeCount = 1;
                tp.Privileges[0].Luid = debugLuid;
                tp.Privileges[0].Attributes = SE_PRIVILEGE_ENABLED;
                NtAdjustPrivilegesToken(tokenHandle, FALSE, &tp, sizeof(tp), NULL, NULL);
    ...
    // --- Stage 4: Handle duplication from a privileged process ---
    if (!NT_SUCCESS(status))
    {
        status = PhpStealProcessHandleFromSystem(
            ProcessHandle,
            DesiredAccess,
            ProcessId
            );
    ...
    // --- Stage 5: KPH at max level (absolute last resort) ---
    if (!NT_SUCCESS(status) && (level == KphLevelMax))
    {
        status = KphOpenProcess(
            ProcessHandle,
            DesiredAccess,
    ...
    )
{
    ...
    // --- Stage 5: KPH at max level (last resort) ---
    if (!NT_SUCCESS(status) && (level == KphLevelMax))
    {
        status = KphOpenProcess(
            ProcessHandle,
